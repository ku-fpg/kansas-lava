...
testAsyncMemory (TestSeq test toL) tyName ws = do
    let (writes,rds) = unzip $ toL ws
...
testSyncMemory :: forall w1 w2 .
                  ...
                  , Size (W (Maybe (w1,w2))))
               => TestSeq -> String -> Gen (Maybe (w1,w2),w1) -> IO ()
testSyncMemory (TestSeq test toL) tyName ws = do
    let (writes,rds) = unzip $ toL ws
...
testMatrixMemory (TestSeq test toL) tyName ws = do
    let writes = toL ws
...
testRomMemory :: forall w1 w2 .
              ...
              -> Gen (w1,w2)
              -> IO ()
testRomMemory (TestSeq test toL) tyName ws = do
    let (addr,vals) = unzip $ toL ws
