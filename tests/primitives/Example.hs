...
-- Imports only necessary for some of my examples below
...
-- To generate a list of inputs/outputs generated by the shallow embedding:
--
    let inp :: Seq U4
        inp  = toSeq $ cycle [0..15]

--  genInput :: (..) => Int -> a -> (a -> b) -> IO [String] (each string is a line/cycle)
    inputs <- genInput 20 (register :: Env () -> Comb U4 -> Seq U4 -> Seq U4) (\ reg -> reg shallowEnv 10 inp)
    mapM_ putStrLn inputs

--  genInfo :: (..) => Int -> a -> (a -> b) -> IO [String] (each string is a line/cycle)
    info <- genInfo 20 (register :: Env () -> Comb U4 -> Seq U4 -> Seq U4) (\ reg -> reg shallowEnv 10 inp)
    mapM_ putStrLn info
